...
describe('slaInfo', () => {
  ...
  it('hatırlatma penceresi içinde kesirli saat yukarı yuvarlanır (ceil)', () => {
    const info = slaInfo(hoursAgo(20.5), SLA_HOURS, REMINDER_HOURS, false, now) // 3.5s kaldı -> ceil 4
    expect(info.state).toBe('warning')
    expect(info.hoursLeft).toBe(4)
  })
