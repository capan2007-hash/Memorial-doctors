...
describe('matchesSearch', () => {
  ...
  it('ada göre (büyük/küçük duyarsız, kısmi)', () => {
    expect(matchesSearch(rows[0], 'ayşe')).toBe(true)
    expect(matchesSearch(rows[0], 'yılmaz')).toBe(true)
    expect(matchesSearch(rows[0], 'mehmet')).toBe(false)
  })
  it('telefona göre (biçim farkı önemsiz — yalnız rakam)', () => {
    expect(matchesSearch(rows[0], '5551234')).toBe(true)
    expect(matchesSearch(rows[0], '0555 123')).toBe(true)
  ...
  it('2 haneden kısa rakam telefon eşleşmesi tetiklemez', () => {
    expect(matchesSearch(rows[0], '05')).toBe(false)
  })
})
