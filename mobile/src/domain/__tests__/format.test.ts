...
describe('timeAgo sınır durumları', () => {
  ...
  it('59 sn → az önce', () => { expect(timeAgo(at(59_000), NOW2)).toBe('az önce') })
  it('59 dk → dk önce', () => { expect(timeAgo(at(59 * 60_000), NOW2)).toBe('59 dk önce') })
  it('23 sa → sa önce', () => { expect(timeAgo(at(23 * 3_600_000), NOW2)).toBe('23 sa önce') })
  it('6 gün → gün önce', () => { expect(timeAgo(at(6 * 86_400_000), NOW2)).toBe('6 gün önce') })
  it('7 gün → tarih formatı', () => { expect(timeAgo(at(7 * 86_400_000), NOW2)).toBe('11.07.2026') })
})
