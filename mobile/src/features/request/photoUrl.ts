// Kaynak: /src/features/requests/photoUrl.ts (web) — M4 Task 4 (FR-37/38).
// Aktif katman fotoğrafları bugünkü gibi doğrudan createSignedUrl ile imzalanır.
// Arşiv katmanı fotoğrafları client'ta ASLA doğrudan imzalanmaz — `photo-url`
// edge fonksiyonu caller yetkisini doğrular (koordinatör/admin veya atanan
// doktor; agent/sales engellenir), kısa ömürlü (120sn) imzalı URL üretir ve
// erişimi audit_log'a yazar. İmha edilmiş fotoğraflar (deleted_at dolu) için
// URL üretilmez. supabase.functions.invoke web istemcisiyle aynı şekilde çalışır.
